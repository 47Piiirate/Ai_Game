...
class AchievementSystem:
    ...
    def check_game_progress_achievements(self, player, game_state):
        ...
        if hasattr(game_state, 'boss_defeated') and game_state.boss_defeated and not self.achievements["boss_slayer"].unlocked:
            self.unlock_achievement("boss_slayer")
        
        # Check for explorer achievement
        if hasattr(game_state, 'discovered_areas') and hasattr(game_state, 'total_areas') and len(game_state.discovered_areas) >= game_state.total_areas:
            self.unlock_achievement("explorer")
        
        # Check for collector achievement
        if hasattr(player, 'collected_items') and hasattr(game_state, 'total_collectibles') and len(player.collected_items) >= game_state.total_collectibles:
